		...
		login.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(Button.ClickEvent event) {
				try {
					...
					// With app servers that can't handle FORWARD properly,
					// previous location is stored into session
					//
					// the double get session is not a typo, there are sessions
					// and vaadin sessions ;-)
					Object origPage = getSession().getSession().getAttribute(
							Servlet.AUTH_ORIGINALPAGE);
					if (origPage != null) {
						getSession().getSession().setAttribute(
								Servlet.AUTH_ORIGINALPAGE, null);
						page.setLocation(origPage.toString());
					} else {
						// Else just reload the page to get the actually
						// accessed UI
						page.setLocation(page.getLocation());
					}
				} catch (Exception ex) {
					Notification.show("FAILED");
					Logger.getLogger(LoginView.class.getName()).log(
